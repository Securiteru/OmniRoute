...
export const CLI_TOOLS: Record<string, CliCatalogEntry> = {
  ...
  droid: {
    ...
    color: "#00D4FF",
    description: "Factory AI Droid — ACP-native agent with configurable endpoint",
...
OPENAI_API_KEY: "{{apiKey}}"`,
  ...
  warp: {
    ...
    icon: "terminal",
    color: "#1D4ED8",
    description: "Warp AI terminal — use Warp/Oz model tokens through a local proxy",
    ...
    guideSteps: [
      ...
      {
        step: 2,
        title: "Start Warp Proxy",
        desc: "Run the warp_proxy server (make dev-warp-proxy) on a reachable host",
      },
      {
        step: 3,
        title: "API Key",
        type: "apiKeySelector",
        desc: "Any dummy value — the proxy handles auth via WARP_API_KEY",
      },
      {
        step: 4,
        title: "Base URL",
        value: "{{baseUrl}}",
        copyable: true,
        desc: "Point to the proxy (e.g. http://100.x.x.x:4444/v1)",
      },
    ...
    notes: [
      {
        type: "info",
        text: "Warp provider uses a local OpenAI-compatible proxy (warp_proxy) that wraps the `oz agent run` CLI. The proxy must be running on a reachable host (Tailscale IP recommended). 89+ models available via `oz model list`.",
      },
    ],
  },
